...
u32 state_cycles = 0;
u32 messages_sent = 0;
u32 messages_num = 0;//yk
int is_timeout_or_sent = 0;//yk
...
void setup_request_and_response_file()
{
  ...
  else if(!strcmp(protocol_name, "FTP"))
  {
...
// yk 请求消息拼接起来，然后保存到文件中。
void request_to_file(s32 fd, u8 *request_file)
{
  printf("[fuzz]1140hang request_to_file start!\n");
  kliter_t(lms) *it;
  u8 *mem = NULL;
  ...
  for(it = kl_begin(kl_messages); it !=kl_end(kl_messages); it = kl_next(it)){
    ...
    mem = (u8 *)ck_realloc(mem, len + message_size + 11);
    memcpy(&mem[len], kl_val(it)->mdata, message_size);
    memcpy(&mem[len + message_size], "\r\nMESSAGE\r\n", 11);
    len = len + message_size + 11;
    messages_num++;
  }
  ck_write(fd, mem, len, request_file);

  printf("[fuzz]messages_num is %d\n",messages_num);
  printf("[fuzz]mem is ");
  for(int i=0;i<len;i++)
  {
    printf("%02x ",mem[i]);
  ...
  close(fd);
  ck_free(mem);
  printf("[fuzz]1148 request_to_file end!\n");
}
// yk

void file_to_response()
{
  s32 response_file_fd;
  FILE *response_size_file;
  messages_sent = 0;
  if(access(response_file, F_OK) == 0)
  {
    response_file_fd = open(response_file, O_RDONLY, 0600);
    if(response_file_fd < 0) PFATAL("Unable to open '%s'", response_file);
  }
  else
  {
    response_bytes = (u32 *) ck_realloc(response_bytes, 1 * sizeof(u32));
    response_bytes[0] = 0;
    messages_sent = 1;
    return;
  }
  if(access(response_size, F_OK) == 0)
  {
    response_size_file = fopen(response_size, "r");
    if(response_size_file == NULL) PFATAL("Unable to open '%s'", response_size);
  }
  response_buf_size = lseek(response_file_fd, 0, SEEK_END);
  lseek(response_file_fd, 0, SEEK_SET);
  response_buf = (char *)ck_alloc(response_buf_size*sizeof(char));
  printf("[fuzz]response_buf_size is %d\n",response_buf_size);
  read(response_file_fd, response_buf, response_buf_size);
  close(response_file_fd);
  // if(!is_timeout_or_sent)
  // {
  //   char line[100];
  //   printf("[fuzz]fgets 11\n");
  //   fgets(line,sizeof(line),response_size_file);
  //   printf("[fuzz]fgets 22\n");
  //   int count = 0;
  //   while(fgets(line,sizeof(line),response_size_file))
  //   {
  //     int num1, num2;
  //     count++;
  //     sscanf(line, "%d-%d", &num1, &num2);
  //     printf("[fuzz]num1 is %d; num2 is %d\n",num1, num2);
  //     response_bytes = (u32 *) ck_realloc(response_bytes, count * sizeof(u32));
  //     response_bytes[num1 - 1] = num2;
  //     messages_sent = num1;
  //   }
  //   fclose(response_size_file);
  // }
  // else
  // {
  //   char line[100];
  ...
  //   if(messages_sent != 0)
  //   {
  //     if(response_bytes[messages_sent - 1] != response_buf_size)
  //     {
  //       count++;
  //       response_bytes = (u32 *) ck_realloc(response_bytes, count * sizeof(u32));
  ...
  //   }
  // }
  // yk 2024.6.21为了mosquitto而改，可能会影响到live555或者pureftpd
  char line[100];
  int num1, num2;
  printf("[fuzz]fgets 1\n");
  fgets(line,sizeof(line),response_size_file);
  ...
  if(messages_sent != 0)
  {
    if(response_bytes[messages_sent - 1] != response_buf_size)
    {
      ...
      int min = ((messages_sent + 1) < messages_num) ? (messages_sent + 1) : messages_num;
      if(min != messages_sent){
        count++;
        response_bytes = (u32 *) ck_realloc(response_bytes, count * sizeof(u32));
        response_bytes[messages_sent] = response_buf_size;
        messages_sent = messages_sent + 1;
  ...
  printf("[fuzz]messages_sent is %d\n",messages_sent);
  printf("[fuzz]一共%d条消息\n",messages_sent);
  for(int i=0;i<messages_sent;i++){
    printf("[fuzz]response_bytes[%d]是%d字节!\n",i ,response_bytes[i]);
  }
}

//yk
int aflnet_communicate_with_equafl()
{
  // Clear the response buffer and reset the response buffer size
  if (response_buf) {
    ck_free(response_buf);
  ...
  *flag_id = 0;
  printf("[fuzz]origin_flag_id is %d\n",*flag_id);
  // 等待一个信号表示所有消息都已经被发送。
  double threshold = 1000 * messages_num * 2 * 10;
  // double threshold = 1000 * messages_num * 2 * 6;
  printf("[fuzz]threahold is %f\n",threshold);
  // 判断是超时还是消息发送完。
  is_timeout_or_sent = 0;
  struct timeval start, current;
  gettimeofday(&start, NULL);
  while(1){
    //yk
    usleep(1000); // 不写这个的话，一直跳不出去while循环。
    gettimeofday(&current, NULL);
    double diff = (current.tv_sec - start.tv_sec)*1000000 + (current.tv_usec - start.tv_usec);
    // printf("[fuzz]diff is %f\n",diff);
    if(diff >= threshold)
    {
      printf("[fuzz]timeout\n");
      is_timeout_or_sent = 1;
      break;
    }
    if(*flag_id)
    {
      printf("[fuzz]flag_id is %d\n",*flag_id);
      *flag_id = 0;
      break;
    }
  }
...
EXP_ST void setup_shm(void) {
  ...
  mem_id = shmget(IPC_PRIVATE, 1, IPC_CREAT | IPC_EXCL | 0600);

  if (shm_id < 0) PFATAL("shmget() failed");
  if (mem_id < 0) PFATAL("shmget() failed");

  atexit(remove_shm);
  atexit(remove_mem);

  shm_str = alloc_printf("%d", shm_id);
  mem_str = alloc_printf("%d", mem_id);
  ...
  if (!dumb_mode)
  {
    ...
    setenv(REQ_SEND_COM, mem_str, 1);
  }
  ck_free(shm_str);
  ck_free(mem_str);

  trace_bits = shmat(shm_id, NULL, 0);
  flag_id = shmat(mem_id, NULL, 0);

  if (!trace_bits) PFATAL("shmat() failed");
  if (!flag_id) PFATAL("shmat() failed");
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    setsid();
//yk  这两句代码含义好像是将标准输出重定向到空设备，暂时注释掉。
    // dup2(dev_null_fd, 1);
    // dup2(dev_null_fd, 2);
//yk
    if (out_file) {
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if(request_file){
    unlink(request_file);
    fd = open(request_file, O_WRONLY | O_CREAT, 0600);
    if(fd < 0) PFATAL("Unable to create '%s'", request_file);
  }
  request_to_file(fd, request_file);
  //yk
  ...
  printf("[fuzz]fuzz start\n");
  if (dumb_mode == 1 || no_forkserver) {
    // if (use_net) send_over_network();
    if(use_net) aflnet_communicate_with_equafl();
    if (waitpid(child_pid, &status, 0) <= 0) PFATAL("waitpid() failed");

  } else {
    // if (use_net) send_over_network();
    // yk 将AFLNET和EQUAFL接起来。
    if(use_net) aflnet_communicate_with_equafl();
    // yk
    s32 res;
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  stage_max = 1; //yk 暂时注释掉，因为update_state_aware_variables()部分代码暂时无法运行。
...
AFLNET_REGIONS_SELECTION:;
  ...
  if (state_aware_mode) {
    ...
    printf("[fuzz]queue_cur region_count is %d\n",total_region);
    if (target_state_id == 0) {
      ...
      printf("[fuzz]target_state_id == 0; M2_region_count is %d\n",M2_region_count);
    } else {
      ...
      printf("[fuzz]target_state_id == %d; M2_region_count is %d\n",target_state_id, M2_region_count);
  ...
  printf("[fuzz]construct messages start!\n");
  ...
  printf("[fuzz]construct messages end!\n");
  ...
  printf("[fuzz]M2(orgin out_buf) is ");
...
#define FLIP_BIT(_ar, _b) do { \
  ...
  printf("[fuzz]Single walking bit.\n");
  /* Single walking bit. */
  stage_short = "flip1";
  stage_max   = len << 3;
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
  ...
  /* Four walking bytes. */
  printf("[fuzz]Four walking bytes.\n");
...
skip_bitflip:
  ...
  printf("[fuzz]16-bit arithmetics, both endians.\n");
  ...
  printf("[fuzz]32-bit arithmetics, both endians.\n");
...
skip_arith:
  ...
  printf("[fuzz]Setting 16-bit integers, both endians.\n");
  ...
  printf("[fuzz]Setting 32-bit integers, both endians.\n");
...
skip_interest:
  ...
  printf("[fuzz]Insertion of user-supplied extras.\n");
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      int random_num = UR(15 + 2 + (region_level_mutation ? 4 : 0));
      switch (random_num) {
    ...
    printf("[fuzz]havoc_stage: modified out_buf is ");
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:z:x:QN:D:W:w:e:P:KEq:s:RFc:l:")) > 0)
  ...
  //yk 设置了请求和响应文件的文件名。
  setup_request_and_response_file();
  //yk
  ...
  if (state_aware_mode) {
    ...
    while (1) {
      ...
      while(!selected_seed || selected_seed->region_count == 0) {
        ...
        printf("[fuzz]target_state_id is %d\n",target_state_id);
        ...
        {
          printf("[fuzz]select seed region_count is %d\n",selected_seed->region_count);
        }
      }
